...
We can see that the actual mean number of steps is `r meansteps` with a median value of
`r mediansteps`. This is close to what we noticed with the histogram data.
...
there are `r nasteps` missing step records out of `r steps` total step records. That is
`r nasteppcent`% of records are missing.
...
# This block cleans up the extracted data files from the zip archive.
# It doesn't necessarly need to be done but it makes me feel better that the working
